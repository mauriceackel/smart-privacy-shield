...
.. code-block:: cmake
  ...
  fixup_plugin(<app> <libs> <dirs>)

Fix up ``<app>`` bundle in-place and make it standalone, such that it can be
drag-n-drop copied to another machine and run on that machine as long
as all of the system libraries are compatible.

If you pass plugins to ``fixup_plugin`` as the libs parameter, you should
install them or copy them into the bundle before calling ``fixup_plugin``.
...
Then clear all the keys and call ``verify_plugin`` on the final bundle to
...
.. code-block:: cmake
  ...
  copy_and_fixup_plugin(<src> <dst> <libs> <dirs>)
...
.. code-block:: cmake
  ...
  get_plugin_dir(<exe> <dotapp_dir_var>)
...
.. code-block:: cmake
  ...
  init_plugin_dir(<app> <bundle_var> <executable_var> <valid_var>)
...
``BU_COPY_FULL_FRAMEWORK_CONTENTS`` to ``ON`` before calling fixup_plugin.  By
default, ``COPY_RESOLVED_FRAMEWORK_INTO_BUNDLE`` copies the framework
dylib itself plus the framework ``Resources`` directory.

.. code-block:: cmake

  fixup_plugin_item(<resolved_embedded_item> <exepath> <dirs>)
...
the bundle already.  In other words, if you pass plugins to ``fixup_plugin``
as the libs parameter, you should install them or copy them into the
bundle before calling ``fixup_plugin``.  The ``libs`` parameter is a list of
...
function(get_plugin_dir plugin plugin_dir_var)
  set(p "${plugin}")

  get_filename_component(plugin_dir "${p}" PATH)

  set(${plugin_dir_var} "${plugin_dir}" PARENT_SCOPE)
endfunction()

function(init_plugin_dir plugin plugin_dir_var plugin_var valid_var)
  set(valid 0)

  if(EXISTS "${plugin}")
    # Is it a directory?
    if(IS_DIRECTORY "${plugin}")
      message(STATUS "warning: *NOT* handled - plugin path is a directory...")
    else()
      get_plugin_dir("${plugin}" plugin_dir)
      if(EXISTS "${plugin_dir}")
          set(${plugin_dir_var} "${plugin_dir}" PARENT_SCOPE)
          set(${plugin_var} "${plugin}" PARENT_SCOPE)
          set(valid 1)
      else()
        message(STATUS "warning: *NOT* handled - plugin folder does not exist...")
      endif()
    endif()
  else()
    message(STATUS "warning: *NOT* handled - file does not exist...")
  endif()

  if(NOT valid)
    set(${plugin_dir_var} "error: not a plugin" PARENT_SCOPE)
    set(${plugin_var} "error: not a plugin" PARENT_SCOPE)
  endif()
...
function(set_bundle_key_values keys_var context item exepath dirs copyflag)
  ...
  if(NOT length_before EQUAL length_after)
    ...
    # User @rpath instead of the default embedded path
    #
    string(REPLACE "${default_embedded_path}" "@rpath" embedded_item "${embedded_item}")
...
function(get_bundle_keys plugin libs dirs keys_var)
  set(${keys_var} PARENT_SCOPE)
  ...
  init_plugin_dir("${plugin}" bundle executable valid)
  if(valid)
    ...
    #
    list(APPEND combined_libs ${libs} ${executable}) # Required as our main plugin is not an executable itself
    foreach(lib ${combined_libs})
      ...
      if(NOT prereq_filename IN_LIST CFG_IGNORE_ITEM)
        ...
        foreach(pr ${prereqs})
          get_filename_component(dependency_name ${pr} NAME)
          # Don't copy file when ignored
          if (dependency_name IN_LIST CFG_IGNORE_ITEM)
            set(copy 0)
          else()
            set(copy 1)
          endif()

          set_bundle_key_values(${keys_var} "${lib}" "${pr}" "${exepath}" "${dirs}" ${copy} "${main_rpaths}")
        endforeach()
      else()
    ...
    foreach(exe ${exes})
      ...
      if(NOT prereq_filename IN_LIST CFG_IGNORE_ITEM)
        ...
        foreach(pr ${prereqs})
          ...
          set_bundle_key_values(${keys_var} "${exe}" "${pr}" "${exepath}" "${dirs}" ${copy} "${exe_rpaths}")
        endforeach()
      else()
        message(STATUS "Ignoring file: ${prereq_filename}")
      endif()
    ...
    foreach(key ${${keys_var}})
      ...
      #   "-- ${key} -- \n"
      #   "ITEM: " "${${key}_ITEM}" "\n"
      #   "RESOLVED_ITEM: " "${${key}_RESOLVED_ITEM}"  "\n"
      #   "DEFAULT_EMBEDDED_PATH: " "${${key}_DEFAULT_EMBEDDED_PATH}"  "\n"
      #   "EMBEDDED_ITEM: " "${${key}_EMBEDDED_ITEM}"  "\n"
      #   "RESOLVED_EMBEDDED_ITEM: " "${${key}_RESOLVED_EMBEDDED_ITEM}"  "\n"
      #   "COPYFLAG: " "${${key}_COPYFLAG}"  "\n"
      #   "RPATHS: " "${${key}_RPATHS}"  "\n"
      #   "RDEP_RPATHS: " "${${key}_RDEP_RPATHS}"  "\n"
      # )
...
function(fixup_plugin_item resolved_embedded_item exepath dirs)
  ...
  #
  get_plugin_dir("${exepath}" exe_dotapp_dir)
  ...
  if(NOT is_embedded)
    ...
    message("")
    message("Install or copy the item into the bundle before calling fixup_plugin.")
    message("Or maybe there's a typo or incorrect path in one of the args to fixup_plugin?")
    message("")
  ...
  foreach(pr ${prereqs})
    ...
    if(NOT "${${rkey}_EMBEDDED_ITEM}" STREQUAL "")
      ...
      message("warning: unexpected reference to '${pr}' for item '${resolved_embedded_item}'")
    endif()
  endforeach()
  ...
  #
  foreach(rpath ${CUSTOM_RPATHS})
    set(changes ${changes} -add_rpath "${rpath}")
  endforeach()
...
function(fixup_plugin plugin libs dirs)
  message(STATUS "fixup_plugin")
  ...
  if(valid)
    ...
    message(STATUS "fixup_plugin: preparing...")
    get_bundle_keys("${plugin}" "${libs}" "${dirs}" keys IGNORE_ITEM "${CFG_IGNORE_ITEM}")

    message(STATUS "fixup_plugin: copying...")
    list(LENGTH keys n)
    math(EXPR n ${n}*2)
    ...
    message(STATUS "fixup_plugin: fixing...")
    foreach(key ${keys})
      math(EXPR i ${i}+1)
      if(APPLE)
        get_filename_component(dependency_name "${${key}_RESOLVED_EMBEDDED_ITEM}" NAME)
        if(NOT dependency_name IN_LIST CFG_IGNORE_ITEM)
          message(STATUS "${i}/${n}: fixing up '${${key}_RESOLVED_EMBEDDED_ITEM}'")
          if(NOT "${${key}_COPYFLAG}" STREQUAL "2")
            fixup_plugin_item("${${key}_RESOLVED_EMBEDDED_ITEM}" "${exepath}" "${dirs}")
          endif()
        else()
          message(STATUS "${i}/${n}: *NOT* fixing up '${${key}_RESOLVED_EMBEDDED_ITEM}' (excluded)")
        endif()
      else()
    ...
    message(STATUS "fixup_plugin: cleaning up...")
    clear_bundle_keys(keys)

    message(STATUS "fixup_plugin: verifying...")
    verify_plugin("${plugin}" IGNORE_ITEM "${CFG_IGNORE_ITEM}")
  else()
    message(SEND_ERROR "error: fixup_plugin: not a valid bundle")
  endif()

  message(STATUS "fixup_plugin: done")
endfunction()


function(copy_and_fixup_plugin src dst libs dirs)
  execute_process(COMMAND ${CMAKE_COMMAND} -E copy_directory "${src}" "${dst}")
  fixup_plugin("${dst}" "${libs}" "${dirs}")
endfunction()
...
function(verify_plugin plugin)
  ...
  message(STATUS "===========================================================================")
  message(STATUS "Analyzing plugin='${plugin}'")
  ...
  if(NOT verified)
    message(FATAL_ERROR "error: verify_plugin failed")
  endif()
endfunction()
